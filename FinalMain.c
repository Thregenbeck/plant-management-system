...
*
*    PERFORMANCE:
*    Main program that runs through the entire State machine of the system. It
* 	 uses all the header files to call functions and transition between all the
* 	 statements.
* 	                              **NOTE**
* 	 This program relies on many files to compile correctly. Below is the command
* 	to compile the code. Refer to documentation to download required files to run.  
*	 gcc -o Main Main.c adcTesting.c pwm.c Initialization.c 7_seg.c 
...
int main(void)
{
	...
	//-Turns-Off-All-LEDS-and-Buzzer-Except-Power-on-Program-Start
	digitalWrite(ManualModeLED, HIGH); //LEDS are Active Low!
	...
	while (1)
	{
		...
		printf("Temp Voltage = %4.2f \n" , TempVoltage);	
		printf("Soil Voltage = %d \n" , SoilVoltage);
		printf("Level Voltage = %d \n" , LevelVoltage);
		...
		//Turns-on-LED-and-PWM-if-Manual-Switch-is-on-and-Sprinkler-button
		//is-pressed.--
		if (digitalRead(SprinklerOn) == LOW && digitalRead(ManualMode) == HIGH)
		{
		...
		//Turn-on-Water-Low-LED-Buzzer-and-7-seg-if-silent-mode-switch
		//off-and-water-level-sensor-reading---------
		...
		//turn-on-PWM-if-Water-level-high-enough-Manual-mode-active-and-soil-is-dry
